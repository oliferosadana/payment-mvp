...
<section class="panel admin-only" id="admin">
...
:root{--ink:#172019;--muted:#68746c;--paper:#fffaf0;--card:#fffdf7;--line:#e6dcc8;--green:#1f7a4d;--red:#b33939;--amber:#a06100;--blue:#225c7a;--shadow:0 18px 50px rgba(57,43,22,.12)}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 8% 0%,#ffe6a3 0,transparent 30%),linear-gradient(135deg,#f5efe1,#e8f0df 55%,#f8e7d4);color:var(--ink);font-family:Georgia,'Times New Roman',serif}.wrap{width:min(100%,1440px);margin:0 auto;padding:18px;min-height:100vh}.hero{display:flex;justify-content:space-between;gap:18px;align-items:center;margin-bottom:22px}.brand{font-size:13px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)}h1{font-size:clamp(28px,4vw,42px);line-height:1;margin:6px 0}h2{margin:0;font-size:24px}h3{margin:18px 0 10px}.pill{display:inline-flex;gap:8px;align-items:center;background:#1d2b22;color:#fff;padding:9px 13px;border-radius:999px;font-family:ui-monospace,monospace;font-size:13px}.quick-nav{position:sticky;top:0;z-index:5;display:flex;gap:8px;overflow:auto;padding:8px 0 14px;background:linear-gradient(180deg,rgba(245,239,225,.96),rgba(245,239,225,.72))}.quick-nav a{white-space:nowrap;text-decoration:none;color:#1d2b22;background:#fffdf7;border:1px solid var(--line);border-radius:999px;padding:8px 12px;font:13px ui-sans-serif,system-ui,sans-serif}.grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:14px;align-items:start}.panel{grid-column:span 12;min-width:0;background:rgba(255,253,247,.94);border:1px solid var(--line);border-radius:22px;padding:16px;box-shadow:var(--shadow);backdrop-filter:blur(10px);scroll-margin-top:64px}.half{grid-column:span 6}.third{grid-column:span 4}.panel-head{display:flex;justify-content:space-between;gap:12px;align-items:start;margin-bottom:14px}.eyebrow{margin:0 0 4px;color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.14em}.form-grid{display:grid;grid-template-columns:minmax(220px,1.3fr) minmax(140px,.8fr) minmax(180px,1fr) auto;gap:10px;align-items:center}.compact{grid-template-columns:1fr 1fr auto}.split{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.3fr);gap:18px}input{width:100%;border:1px solid var(--line);background:#fff;border-radius:14px;padding:12px 13px;font:16px Georgia,'Times New Roman',serif}button{border:0;border-radius:14px;background:#1d2b22;color:#fff;padding:12px 15px;cursor:pointer;font-weight:700}button.secondary{background:#e9ddc7;color:#1d2b22}.result{white-space:pre-wrap;max-height:180px;overflow:auto;background:#1d2b22;color:#dff4df;border-radius:16px;padding:12px;display:none}.stats{display:grid;grid-template-columns:repeat(5,1fr);gap:12px}.stat{background:#f7eddb;border:1px solid var(--line);border-radius:18px;padding:14px}.stat b{display:block;font-size:30px}.table-wrap{overflow:auto;max-height:430px;border:1px solid var(--line);border-radius:18px;background:#fff}table{width:100%;border-collapse:collapse;font-size:14px;font-family:ui-sans-serif,system-ui,sans-serif}th{position:sticky;top:0;background:#f4ead8;text-align:left;font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#6b5d4a}td,th{padding:10px;border-bottom:1px solid #eee4d1;vertical-align:top}td{max-width:240px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}td:hover{white-space:normal;overflow:visible}.token-cell{font-family:ui-monospace,monospace;white-space:normal;overflow:visible;text-overflow:clip;word-break:break-all;min-width:360px;max-width:520px}.actions{white-space:normal;min-width:190px}.status{font-weight:800}.paid,.matched,.success,.active,.online{color:var(--green)}.pending,.needs_review{color:var(--amber)}.failed,.unmatched,.disabled,.offline,.cancelled{color:var(--red)}.empty{padding:18px;color:var(--muted);background:#fff;border-radius:18px;border:1px dashed var(--line)}.actions button{padding:8px 10px;margin:2px;font-size:12px}.toast{position:fixed;right:22px;bottom:22px;background:#1d2b22;color:#fff;padding:14px 16px;border-radius:16px;box-shadow:var(--shadow);display:none}@media(max-width:1100px){.half,.third{grid-column:span 12}.split{grid-template-columns:1fr}.form-grid,.compact{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}}@media(max-width:640px){.hero{display:block}.stats{grid-template-columns:1fr}.wrap{padding:12px}.panel{padding:13px;border-radius:18px}.table-wrap{max-height:360px}}</style></head><body><div class="wrap"><header class="hero"><div><p class="brand">Payment SaaS</p><h1>Merchant Operations</h1><span class="pill">Role: ROLE | Merchant: MERCHANT</span></div><button class="secondary" onclick="logout()">Logout</button></header><nav class="quick-nav"><a href="#overview">Overview</a><a href="#invoice">Invoice</a><a href="#qris">QRIS</a><a href="#admin">Admin</a><a href="#invoices-list">Invoices</a><a href="#events-list">Events</a><a href="#devices-list">Devices</a><a href="#callbacks-list">Callbacks</a></nav><main class="grid"><section class="panel" id="overview"><div class="panel-head"><div><p class="eyebrow">Overview</p><h2>Realtime Stats</h2></div><button class="secondary" onclick="load()">Refresh</button></div><div id="stats" class="stats"></div></section><section class="panel half" id="invoice"><p class="eyebrow">Invoice</p><h2>Create Invoice</h2><div class="form-grid"><input id="external_id" placeholder="External ID"><input id="amount" type="number" placeholder="Amount"><input id="customer_name" placeholder="Customer name"><button onclick="createInvoice()">Create</button></div><pre id="invoiceResult" class="result"></pre></section><section class="panel half"><p class="eyebrow">Integration</p><h2>Callback Setting</h2><div class="form-grid compact"><input id="callback_url" placeholder="https://merchant.app/callback"><input id="callback_secret" placeholder="Callback secret"><button onclick="saveCallback()">Save</button></div><pre id="callbackResult" class="result"></pre></section><section class="panel" id="qris"><p class="eyebrow">QRIS</p><h2>Static to Dynamic QRIS</h2><div class="form-grid"><input id="qris_static" placeholder="Paste string QRIS statis"><input id="qris_amount" type="number" placeholder="Amount"><input id="qris_note" placeholder="Catatan opsional" disabled><button onclick="generateQris()">Generate</button></div><pre id="qrisResult" class="result"></pre></section>ADMIN_TOOLS<section class="panel" id="invoices-list"><div class="panel-head"><div><p class="eyebrow">Invoices</p><h2>Recent Invoices</h2></div></div><div id="invoices"></div></section><section class="panel" id="events-list"><p class="eyebrow">Payments</p><h2>Payment Events</h2><div id="events"></div></section><section class="panel half" id="devices-list"><p class="eyebrow">Android</p><h2>Devices</h2><div id="devices"></div></section><section class="panel half" id="callbacks-list"><p class="eyebrow">Webhook</p><h2>Callback Attempts</h2><div id="callbacks"></div></section></main></div><div id="toast" class="toast"></div><script>
...
function cell(v,c){if(typeof v==='string'&&v.startsWith('<button'))return v;if(c==='token'&&v)return '<code class="token-cell">'+esc(v)+'</code><br><button onclick="copyText(&quot;'+esc(v)+'&quot;)">copy</button>';if(typeof v==='object'&&v!==null)v=JSON.stringify(v);return esc(v)}
function table(rows){if(!rows||!rows.length)return '<div class="empty">No data yet</div>';let cols=Object.keys(rows[0]);return '<div class="table-wrap"><table><tr>'+cols.map(c=>'<th>'+esc(c)+'</th>').join('')+'</tr>'+rows.map(r=>'<tr>'+cols.map(c=>'<td class="'+esc(String(r[c]))+' '+(c==='action'?'actions':'')+' '+(c==='token'?'token-td':'')+'">'+cell(r[c],c)+'</td>').join('')+'</tr>').join('')+'</table></div>'}
...
async function copyText(v){try{await navigator.clipboard.writeText(v);toast('Token copied')}catch(e){prompt('Copy token',v)}}
